...
struct BlockFactory {
    ...
    static func createBlockNode(from block: Block) -> SKSpriteNode {
        let node = SKSpriteNode(texture: SKTexture(imageNamed: block.imageName))
        node.size = block.size
        node.physicsBody = createPhysicsBody(from: block) // ✅ 실루엣 블록과 차별화
        return node
    }
    
    /// `Block` 모델을 기반으로 **실루엣 블록 노드**를 생성하는 함수 (반투명 효과 적용, 물리 속성 없음)
    static func createSilhouetteBlock(from block: Block) -> SKSpriteNode {
        let node = SKSpriteNode(texture: SKTexture(imageNamed: block.imageName))
        node.size = block.size
    ...
    private static func createPhysicsBody(from block: Block) -> SKPhysicsBody {
